...
void draw(void)
{
    //Loops around the rows in the grid
    for(int row = 0; row < DIMENSION; row++)
    {   
        //Loops around the columns in the grid
        for(int col = 0; col < DIMENSION; col++)
        {
            //For the row/column it is in, it will print the appropriate grid letter
            printf("%c ", grid[row][col]);
        }
        //It will print a new line when it is time to move onto a new row
        printf("\n");    
    }
...
bool lookup(string word)
{
    //define new integers that are needed
    int low = 0;
    int high = dictionary.size - 1;
    
    {   //this loop assumes that the high is above the low, otherwise returns false
        while (low<=high)
        {
            //defines the middle variable that will be searched for
            int mid = ((low+high)/2);
            
            //Compares the strings and responds to a match
            if(strcmp(word,dictionary.words[mid].letters) == 0)
            {   
                //if the word was already found, it doesn't count
                if(dictionary.words[mid].found == true)
                {
                    return false;
                }
                
                //if the word wasn't already found, then it is set to true, so not to be recounted
                dictionary.words[mid].found = true;
                
                //finally returns true to count the points
                return true;
                
            }
            
            //Compares the strings and responds to string compare equating less than 0
            else if(strcmp(word,dictionary.words[mid].letters) < 0)
            {
                //sets the new high to the old mid -1 to cut the search in half
                high = mid - 1;
            }
            
            //Compares the strings and responds to string compare equating more than 0
            else if(strcmp(word,dictionary.words[mid].letters) > 0)
            {
                //sets the new low to the old mid +1 to cut the search in half
                low = mid + 1;
            }
        }
    }
...
void scramble(void)
{
    //defines a new grid that will be generated when scrambled
    char scrambled_grid[DIMENSION][DIMENSION];
    
    //takes the new grid and arranges it in accordance with the old grid
    for(int row = 0; row < DIMENSION; row++)
    {   
        for(int col = 0; col < DIMENSION; col++)
        {
            scrambled_grid[col][DIMENSION-row-1] = grid[row][col];
        }
    }
    
    //replaces the array inside the old grid with the array in the new scrambled grid
    for(int row = 0; row < DIMENSION; row++)
    {   
        for(int col = 0; col < DIMENSION; col++)
        {
            grid[row][col] = scrambled_grid[row][col];
        }
    }
}
